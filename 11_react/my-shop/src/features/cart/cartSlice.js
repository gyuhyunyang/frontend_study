...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    increaseCount: (state, action) => { // 현재의 상태와 액션객체
      const targetItem = state.cartList.find((cart) => cart.id === action.payload ); // cart = cartList에 요소 하나하나 
      targetItem.count += 1;

    },
    decreaseCount: (state, {payload: id} ) => {
      const targetItem = state.cartList.find((cart) => cart.id === id ); // cart = cartList에 요소 하나하나 
      targetItem.count -= 1;
    },
    ...
    addItemToCart: (state, { payload: item }) => {
      ...
      if (targetItem) {
        ...
        // state.cartList.push({id: , title: , price, count: });
